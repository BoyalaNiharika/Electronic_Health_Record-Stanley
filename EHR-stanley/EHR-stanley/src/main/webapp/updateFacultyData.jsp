...
<body>
    ...
    <%
        ...
        String query = "UPDATE facultyapplicationdetails SET Age=?, Weight=?, height=?, BloodGroup=?, Diabetic=?, BloodPressure=?, AllergyHistory=?, Accident=?, TypeofInjury=?, Hospitalized=?, HealthIssues=?, BloodTransfusion=?, EyeSight=?, EyePower=?, Asthama=?, FrequentCold=?, TestedCovidPositive=?, Vaccinated=?, VaccineDoses=?, TypeofCovidVaccine=?, GastricProblem=?, Acidity=?, DonotedBloodBefore=?, How_often_DonatedBlood=?, Year_of_DonatedBlood=?, CardioVascular=?, CardioProblem=?, Ecosprin=?, medication=?, Arthritis=? WHERE Email=?";
